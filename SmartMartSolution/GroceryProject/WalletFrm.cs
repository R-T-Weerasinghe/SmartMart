...
namespace Esoft_last_project
{
    public partial class WalletFrm : Form
    {
        ...
        SqlConnection con2 = new SqlConnection(@"Data Source=RAVINDU-PC;Initial Catalog=GroceryDB;Integrated Security=True");
        ...
        private void WalletFrm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string querySelectPrev = "SELECT Value FROM OrderTBL WHERE PlacedBy=" + UserID + "";


                SqlCommand cmndSelectBalance = new SqlCommand(querySelectBalance, con);
                SqlCommand cmndSelectPrev = new SqlCommand(querySelectPrev, con);
                ...
                con.Open();
                SqlDataReader r = cmndSelectPrev.ExecuteReader();
                while(r.Read())
                {
                    PrevLbl.Text=r[0].ToString();
                }
